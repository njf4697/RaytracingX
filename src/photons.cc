...
#pragma omp parallel for
    ...
    { // create 4-vector \chi parallel to geodesic and fill geodesic initial conditions for each pixel (see https://arxiv.org/pdf/1410.777)
      ...
      CCTK_REAL a_adj = (2.0 * (j + 0.5) / num_pixels_width - 1.0) * tan(alpha_h / 2.0);  // a_{adj} = (2a-1)tan(\alpha_h/2)
      CCTK_REAL b_adj = (2.0 * (i + 0.5) / num_pixels_height - 1.0) * tan(alpha_v / 2.0); // b_{adj} = (2b-1)tan(\alpha_v/2)
